...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    await newCart.save();
    res.send({ result: "Success", message: "Carrito creado con exito" });
  } catch (error) {
    res.send({
      result: "Error",
...
router.put("/:cid", async (req, res) => {
  ...
  if (!id || !name || !category || !price || !stock || !image) {
    response.send({ status: error, error: "Completar los parámetros" });
  }
...
router.delete("/:cartId/products/:pId", async (req, res) => {
  ...
  try {
    ...
    if (!cart) {
      return res.send({ result: "Error", message: "Carrito no encontrado." });
    }
